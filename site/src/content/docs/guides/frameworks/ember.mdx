...
description: How to use the Algebraic Color System in Ember (GTS).
---

This guide shows you how to integrate the Algebraic Color System into a modern Polaris Ember application (using `.gts` components and Vite).
...
pnpm exec color-system build --out ./app/styles/theme.css
pnpm exec color-system export --format typescript --out ./app/utils/theme.ts
```
...
import {
  ThemeManager,
  type ThemeMode,
} from "@algebraic-systems/color-system/browser";
...
The Algebraic Color System is designed to be flexible.

### Build Time (Recommended)

The `color-system build` command generates a static CSS file. This is the most performant option.
...
```ts
import { generateTheme, injectTheme } from '@algebraic-systems/color-system/runtime';
import type { SolverConfig } from '@algebraic-systems/color-system/types';
