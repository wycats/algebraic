...
- [ ] **High Contrast Simulation**:
  ...
  - _Strategy_: Update `LiveThemeInjector` to optionally generate and append the HC block when a toggle is active.
...
- [ ] **`ContextVisualizer`**:
  ...
  - Goal: Show how the _same_ surface slug (`card`) adapts to its parent context.
- [ ] **`GamutComparator`**:
  ...
  - _Implementation_: Use `color-gamut` media queries or a forced sRGB calculation for the "Standard" side.
- [ ] **`LightnessScale`**:
